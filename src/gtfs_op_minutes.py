...
def get_trip_times_new(directory: str)-> pd.DataFrame:
    ...
    trip_times.to_csv(f'{directory}/trip_times.txt', index=False)

    return trip_times

# For trip times which have already been generated into directory folder
def get_trip_times_old(directory: str)->pd.DataFrame:
    return pd.read_csv(f'{directory}/trip_times.txt')

# trip_times = get_trip_times_new(directory)
trip_times = get_trip_times_old(directory)
# print(new_df.iloc[0])
# print("generated:")
# print(gen_df.iloc[0])
...
trip_times_with_trip_info.drop(trip_times_with_trip_info[trip_times_with_trip_info['service_id'] != regular_weekday_service_id].index, inplace=True)

trip_times_with_trip_info.to_csv(f'{directory}/trip_times_with_trip_info.txt', index=False)
...
sorted_route_route_op_mins = dropped_above500.sort_values(by='route_short_name')
sorted_route_route_op_mins.to_csv(f'{directory}/route_operating_minutes.txt', index=False)